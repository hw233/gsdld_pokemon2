...
def use_list(use):
    global loads, load
    loads = functools.partial(loads, use_list=use, encoding="utf-8")
...
def initLog():
    ...
    handler = logging.FileHandler("rpc_"+app.name+".log")
...
class RpcService(object):
    ...
    def __init__(self, svr, sock, uid, size=None):
        ...
        #self.iter_id = itertools.cycle(range(MAX_INDEX))
        self._next_id = 0
        self._resps = {}
    ...
    def stop(self):
        ...
        if 1 and not self.sock_error:
            try:
                printf('call remote_stop:%s', self.sock_addr)
    ...
    def _stop_proxys(self):
        if not len(self._proxys):
            return
        proxys = list(self._proxys.values())
        self._proxys.clear()
        for p in proxys:
            p.on_close()
...
##            self._recv_task.kill(exception=socket_error('reconnect'))
    ...
    def send_imme(self, *args):
        data = dumps(args)
        senddata = b'%s%s' %(pack('I', len(data)), data)
        with self._slock:
            try:
...
##        self._send_queue.put(dumps(args))
    ...
    def _read_response(self, index, timeout):
        ...
        if isinstance(resp, RpcCallError):
            raise resp
    ...
    def call(self, obj_id, name, args, kw, no_result=False,
            timeout=CALL_TIMEORUT, ispickle=False, proxy=False):
        dtype = RT_REQUEST
        if proxy:
            ...
            if isinstance(objs, (tuple, list)):
                ...
                for o in objs:
                    ...
                    ispickle = ispickle or is_pickle
                    obj_ids.append(obj)
            else:
        ...
        if ispickle:
            dtype |= DT_PICKLE
            argkw = pickle.dumps((args, kw), PICKLE_PROTOCOL)
        else:
            try:
                argkw = dumps((args, kw))
            except:
    ...
    def _handle_request(self, parts):
        ...
        try:
            ...
            if not getattr(func, '_rpc_pickle_result_', False):
                ...
                self.send(RT_RESPONSE | DT_PICKLE, index, pickle.dumps(rs, PICKLE_PROTOCOL))
            ...
            #self.send(RT_EXCEPTION, index, pickle.dumps(e, PICKLE_PROTOCOL))
    ...
    def _handle_exception(self, parts):
        ...
        #except:
        error = RpcCallError(error.decode())
        try:
            rs = self._resps.pop(index)
            rs.set(error)
    ...
    def handshake_cli(self):
        self.sock.sendall(self.uid.encode(encoding="utf-8"))
...
class RpcClient(RpcBase):
    ...
    def execute(self, func, args, kw, **others):
        return self.svc.call('', 'execute', (func, args, kw), ispickle=True, **others)
...
class RpcProxy(object):
    ...
    def __call__(self, *args, **kw):
        no_result, timeout, ispickle, proxy = parse_kw(kw, self.timeout)
        return self._svc.call(self._id, '__call__', args, kw,
            no_result=no_result, timeout=timeout, ispickle=ispickle, proxy=proxy)
...
def map_items(proxys, attr, *args, **kw):
    ...
    if not len(proxys):
        return True, []
    no_result, timeout, ispickle, proxy = parse_kw(kw)
    p0 = proxys[0]
...
def uninit():
    ...
    clients = list(_addr_clients.values())
    _addr_clients.clear()
    servers = list(_my_servers_.values())
    _my_servers_.clear()
    for cli in clients:
        cli.stop()
    for svr in servers:
...
def get_addr(export_or_proxy):
    ...
    for addr, svr in _my_servers_.items():
